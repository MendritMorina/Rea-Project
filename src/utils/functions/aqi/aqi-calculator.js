...
const calculateSubIndex = (arr = [], pollutant) => {
  ...
  const filtered = arr.filter(({ datetime, value }) => value && now.diff(dayjs(datetime), 'hours') < hourDiff);
  ...
  // Linear algebra 101
  const aqi = ((aqiHi - aqiLo) / (pollutantHi - pollutantLo)) * (mean - pollutantLo) + aqiLo;
  return aqi;
};
